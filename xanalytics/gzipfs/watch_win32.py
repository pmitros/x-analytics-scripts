"""
xanalytics.gzipfs.watch_win32
===================

Change watcher support for GZIPFS, using ReadDirectoryChangesW on win32.
...
class GZIPFSWatchMixin(WatchableFSMixin):
    ...
    def close(self):
        super(GZIPFSWatchMixin,self).close()
    ...
    def add_watcher(self,callback,path="/",events=None,recursive=True):
        w = super(GZIPFSWatchMixin,self).add_watcher(callback,path,events,recursive)
    ...
    def del_watcher(self,watcher_or_callback):
        ...
        for watcher in watchers:
            for wobj in watcher._watch_objs:
                wt.del_watcher(wobj)
            super(GZIPFSWatchMixin,self).del_watcher(watcher)
        ...
        if self.__watch_thread is None:
            ...
            try:
                if self.__watch_thread is None:
                    ...
                    wt.start()
                    wt.ready.wait()
                    GZIPFSWatchMixin.__watch_thread = wt
            finally:
    ...
    def __shutdown_watch_thread(self,force=False):
        ...
        self.__watch_lock.acquire()
        try:
            if GZIPFSWatchMixin.__watch_thread is None:
                return
            if not force and GZIPFSWatchMixin.__watch_thread.watched_directories:
                return
            try:
                GZIPFSWatchMixin.__watch_thread.close()
            except EnvironmentError:
                pass
            else:
                GZIPFSWatchMixin.__watch_thread.join()
            GZIPFSWatchMixin.__watch_thread = None
        finally:
            self.__watch_lock.release()
