"""
xanalytics.gzipfs.watch_inotify
=============

Change watcher support for GZIPFS, backed by pyinotify.
...
class GZIPFSWatchMixin(WatchableFSMixin):
    ...
    def add_watcher(self,callback,path="/",events=None,recursive=True):
        super_add_watcher = super(GZIPFSWatchMixin,self).add_watcher
    ...
    def __get_watch_thread(self):
        ...
        if GZIPFSWatchMixin.__watch_thread is None:
            GZIPFSWatchMixin.__watch_thread = SharedThreadedNotifier()
            GZIPFSWatchMixin.__watch_thread.start()
        return GZIPFSWatchMixin.__watch_thread


class SharedThreadedNotifier(threading.Thread):
    """pyinotifer Notifier that can manage multiple WatchManagers.

    Each watcher added to an GZIPFS corresponds to a new pyinotify.WatchManager
